...
const InventoryPane = ({ items = defaultInventory, className = '' }: InventoryPaneProps) => (
  <div className={`flex-1 overflow-auto ${className}`.trim()}>
    <table className="min-w-full text-left text-sm" aria-label="Available inventory levels by item">
      ...
      <tbody className="sm:divide-y sm:divide-slate-100 dark:sm:divide-slate-800">
        {items.length === 0 ? (
          <tr>
            <td className="px-0 py-6 text-center text-sm text-slate-500" colSpan={3}>
              No inventory data connected. Add stocked lines to view readiness.
            </td>
          </tr>
        ) : (
          items.map((item) => {
            ...
            return (
              ...
              >
                ...
                <td className="mt-3 flex items-center justify-between text-base sm:mt-0 sm:table-cell sm:text-right">
                  <span className="text-xs font-medium uppercase tracking-wide text-slate-500 sm:hidden">Quantity</span>
                  <span
                ...
                <td className="mt-3 flex items-center justify-between text-base text-slate-600 dark:text-slate-300 sm:mt-0 sm:table-cell sm:text-right">
                  <span className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400 sm:hidden">Unit</span>
