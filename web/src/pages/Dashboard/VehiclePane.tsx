    ...
    <section className={containerClasses}>
      {showHeader && (
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-100 pb-4 dark:border-slate-800">
          ...
          <div className="text-right text-xs leading-5 text-slate-500">
            ...
            <p>{hasVehicles ? 'Live telemetry' : 'Awaiting telemetry feed'}</p>
          </div>
        </div>
      )}
      ...
      <div className="mt-5 flex-1 space-y-4 overflow-y-auto pr-1">
        {hasVehicles ? (
          ...
          <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-6 text-center text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-300">
            No vehicles are connected yet. Add fleet data to see live assignments.
          </div>
        )}
      </div>
    </section>
